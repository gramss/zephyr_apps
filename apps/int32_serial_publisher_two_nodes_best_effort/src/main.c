...
{	
	...
	RCCHECK(rcl_node_init(&node2, "node2", "", &context, &node_ops2))

	//publisher 1
	rcl_publisher_options_t publisher_ops = rcl_publisher_get_default_options();
	publisher_ops.qos.reliability = RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT;
	rcl_publisher_t publisher = rcl_get_zero_initialized_publisher();
	RCCHECK(rcl_publisher_init(&publisher, &node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), "zephyr_int32_publisher1", &publisher_ops))
	...
	RCCHECK(rcl_publisher_init(&publisher2, &node2, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), "zephyr_int32_publisher2", &publisher_ops2))
